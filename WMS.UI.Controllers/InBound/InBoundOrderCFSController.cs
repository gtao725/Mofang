...
{
    public class InBoundOrderCFSController : Controller
    {
        ...
        public void CheckUnitName()
        {
            ...
            entity.SoNumber = Request["SoNumber"].Trim().Replace("\r\n", "").Replace("\n", "").Replace("\r", "").Replace(@"""", "").Replace(@"'", "");
            entity.OrderType = "CFS";
        ...
        {
            string result = cf.InBoundOrderListAdd(entity);
            if (result.Substring(0, 1) == "N")
            {
                Response.Write(result.Substring(1, result.Length - 1));
            }
        ...
        //一键删除SO预录入
        [HttpGet]
        public ActionResult DelInBoundBySO()
        {
            string result = cf.DelInBoundBySO(Session["whCode"].ToString(),Request["clientCode"].ToString(), Request["soNumber"].ToString());
            if (result == "Y")
            {
                return Helper.RedirectAjax("ok", "删除成功！", null, "");
            }
