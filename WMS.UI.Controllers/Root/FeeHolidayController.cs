...
{
    public class FeeHolidayController : Controller 
    {
        ...
        public ActionResult List()
        {
            ...
            int total = 0;
            List<WCF.RootService.FeeHoliday> list = cf.FeeHolidayList(entity, out total).ToList();

            Dictionary<string, string> fieldsName = new Dictionary<string, string>();
            fieldsName.Add("Id", "操作");
            fieldsName.Add("HolidayName", "节假日");
            fieldsName.Add("DayBegin", "日期");
            fieldsName.Add("Type", "倒休");
        ...
        public ActionResult imports()
        {
            ...
            string result = cf.FeeHolidayImports(holiday, dayBegin, type, Session["whCode"].ToString(), Session["userName"].ToString());
            if (result == "")
            {
        ...
        {
            WCF.RootService.FeeHoliday entity = new WCF.RootService.FeeHoliday();
            ...
            string result = cf.FeeHolidayEdit(entity);
            if (result == "Y")
            {
        ...
        {
            int result = cf.FeeHolidayDel(Convert.ToInt32(Request["Id"]));
            if (result >0)
            {
